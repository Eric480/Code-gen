...
$coursecode = $deg_id.$Revision_ID.$code."/"."CO".$selectedSemester.$variable_type.$selectedSlot;
...
//Writing the data into the course table
        ...
        $queryGetCourseID = "SELECT CourseID FROM course WHERE CourseCode = '$coursecode'";
$resultGetCourseID = mysqli_query($conn, $queryGetCourseID);
$rowGetCourseID = mysqli_fetch_assoc($resultGetCourseID);
$course_ID = $rowGetCourseID['CourseID'];

// Insert the data into the "common" table for each combination of Course ID, Programme ID, and Semester
foreach ($selectedDepartment as $offering_dept) {
    // Assuming you have appropriate columns in the "common" table to store the Course ID, Programme ID, and Semester
    $queryCommon = "INSERT INTO common_dept (CourseID,Off_dep) VALUES ('$course_ID', '$offering_dept')";
    mysqli_query($conn, $queryCommon);
}          
